...
var Browser = {
    ...
    saveMedia: function browser_saveMedia(url, type) {
        ...
        function storeBlob(blob, name, retryCount) {
            ...
            addreq.onsuccess = function() {
                ...
                // set wallpaper test
                // JavaScript error: http://browser.gaiamobile.org:8080/js/browser.js, line 1062: too much recursion

                // System JS : ERROR (null):0 - uncaught exception: No permission to call set
                // JavaScript error: http://browser.gaiamobile.org:8080/js/browser.js, line 1006: NS_ERROR_UNEXPECTED: 
    ...
    // Save a media file (image) and Set as wallpaper
    setWallpaper: function browser_setWallpaper(url, type) {
        // top half
        // design 1
        // should be able to get feedback from saveMedia ...
        // invoke set wallpaper (use latest one from gallery?)

        // design 2
        // modify saveMedia, add 1 parameter to allow set as wallpaper

        this.saveMedia(url, type);

        // bottom half
        // set as wallpaper
        // design 1
        // ensure modularity, integrity, use MozActivity
        ...
        switch (nodeName) {
            ...
            case 'IMG':
                return [{
                    label: _('save-' + 'image'),
                    callback: function() {
                        self.saveMedia(item.data.uri, 'image');
                    }
                }, {
                    label: _('set-wallpaper'),
                    callback: function() {
                        self.setWallpaper(item.data.uri, 'image');
                    }
                }];
            case 'VIDEO':
            case 'AUDIO':
